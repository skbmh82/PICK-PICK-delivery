...
// ── 배달비 예시 미리보기 (거리 할증) ──────────────────
function DeliveryFeeExample({ baseKm, baseFee, unitKm, unitFee }: {
  baseKm: number; baseFee: number; unitKm: number; unitFee: number;
}) {
  const fmtKm = (v: number) => (Number.isInteger(v) ? String(v) : v.toFixed(1));
  const cfg = { baseKm, baseFee, surchargeUnitKm: unitKm, surchargeFee: unitFee };

  const rows: { label: string; km: number }[] = [];
  if (baseKm > 0) rows.push({ label: `~${fmtKm(baseKm)}km (기본)`, km: baseKm });
  if (unitKm > 0) {
    for (const mult of [1, 2, 3]) {
      const km = baseKm + unitKm * mult;
      if (km <= MAX_DELIVERY_KM) rows.push({ label: `${fmtKm(km)}km`, km });
    }
  }

  return (
    <div className="mt-3 bg-pick-bg rounded-2xl border border-pick-border px-3 py-2.5">
      <p className="text-[10px] font-black text-pick-purple mb-1.5">💡 예상 배달비 미리보기</p>
      <div className="flex flex-col gap-1">
        {rows.map((r) => (
          <div key={r.label} className="flex justify-between text-[11px]">
            <span className="text-pick-text-sub">{r.label}</span>
            <span className="font-black text-pick-text">
              {calcDeliveryFee(r.km, cfg).toLocaleString()}원
            </span>
          </div>
        ))}
        <div className="flex justify-between text-[11px] border-t border-pick-border/60 pt-1 mt-0.5">
          <span className="text-pick-text-sub">{MAX_DELIVERY_KM}km 초과</span>
          <span className="font-black text-red-500">배달 불가</span>
        </div>
      </div>
    </div>
  );
...
export default function StoreSettingsPage() {
  ...
  const [deliverySurchargeFee,    setDeliverySurchargeFee]    = useState("1000");
  ...
  const fetchStore = useCallback(async () => {
    ...
    try {
      ...
      const data: StoreData = {
        ...
        deliverySurchargeFee:    Number(s.delivery_surcharge_fee ?? 1000),
      ...
      setDeliverySurchargeFee(String(data.deliverySurchargeFee));
  ...
  const handleSave = async () => {
    ...
    const surFee  = parseInt(deliverySurchargeFee, 10);
    ...
    <div className="min-h-screen bg-pick-bg pb-28">
      ...
      <div className="px-4 pt-4 space-y-4">
        ...
        <div className="bg-white rounded-3xl p-5 shadow-sm border border-pick-border">
          ...
          <Field label="기본 배달비 (원)" icon={<Wallet size={12} />}>
            <p className="text-[11px] text-pick-text-sub mb-2">
              아래 <strong>기본 구간</strong> 거리 이내는 이 배달비가 적용돼요
            </p>
            <input
              value={deliveryFee}
          ...
          {/* 거리 할증 설정 */}
          <Field label="거리 할증 설정" icon={<Truck size={12} />}>
            <p className="text-[11px] text-pick-text-sub mb-2 leading-relaxed">
              기본 구간을 넘는 거리는 <strong>단위 거리마다 할증</strong>이 붙어요.
              최대 배달 거리는 <strong>20km</strong>이며 초과 시 배달 불가예요.
            </p>
            <div className="grid grid-cols-3 gap-2">
              <div>
                <p className="text-[10px] font-bold text-pick-text-sub mb-1 text-center">기본 구간(km)</p>
                <input
              ...
              <div>
                <p className="text-[10px] font-bold text-pick-text-sub mb-1 text-center">할증 단위(km)</p>
                <input
                  value={deliverySurchargeUnitKm}
                  onChange={(e) => setDeliverySurchargeUnitKm(e.target.value.replace(/[^0-9.]/g, ""))}
                  className={`${numInputCls} text-center`}
                  placeholder="2"
                  inputMode="decimal"
                />
              ...
              <div>
                <p className="text-[10px] font-bold text-pick-text-sub mb-1 text-center">단위당(원)</p>
                <input
                  value={deliverySurchargeFee}
                  onChange={(e) => setDeliverySurchargeFee(e.target.value.replace(/\D/g, ""))}
                  className={`${numInputCls} text-center`}
                  placeholder="1000"
                  inputMode="numeric"
                />
            ...
            {/* 실시간 예시 */}
            <DeliveryFeeExample
              baseKm={parseFloat(deliveryBaseKm) || 0}
              baseFee={parseInt(deliveryFee, 10) || 0}
              unitKm={parseFloat(deliverySurchargeUnitKm) || 0}
              unitFee={parseInt(deliverySurchargeFee, 10) || 0}
            />
          </Field>
