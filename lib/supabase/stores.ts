...
export async function fetchStoreById(id: string): Promise<StoreRow | null> {
  ...
  const { data, error } = await supabase
    .from("stores")
    .select("id, name, category, description, notice, address, phone, lat, lng, image_url, banner_url, rating, review_count, delivery_time, delivery_fee, min_order_amount, is_open, delivery_radius_km, delivery_base_km, delivery_surcharge_unit_km, delivery_surcharge_fee")
    .eq("id", id)
    .eq("is_approved", true)
    .single();
